...
public class DownloaderTaskFragment extends Fragment {
	...
	private String[] downloadTweets(Integer resourceIDS[]) {
		...
		try {
			for (int idx = 0; idx < resourceIDS.length; idx++) {
				...
				feeds[idx] = buf.toString();//got the tweet here by using aync task
	...
	private void notify(final boolean success) {
		...
		mContext.sendOrderedBroadcast(new Intent(
						MainActivity.DATA_REFRESHED_ACTION), null,         //intent to receive the broadcast intent
				new BroadcastReceiver() {                          //null--> no permission required to accept the broadcast
